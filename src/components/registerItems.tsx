...
class RegisterItem extends React.Component {
    ...
    render() {
      ...
      console.log(this.props.data.node.title)
        return (
          <GridItem aria-label={`View project "${this.props.data.node.title}"`}>
          ...
          if (props.remove && e.node.id === props.remove) return;
            fileIndex = props.data.allFile.edges.find(({node}) => {
              if (node.parent) {
                ...
                if (node.parent.id == `SitePage /${lang}/register/` + e.node.slug) {
                  return node
                }
              }
            })
            ...
            if (fileIndex) {
              items.push(<RegisterItem key={e.node.id} data={e} file={fileIndex}/>);
            }
